...
const CalendarStart = ({ onDayClick, setStartOpen }: CalendarStartType) => {
  ...
  return (
    <div className="searchroomcalendarstart">
      <div className="searchroomcalendarstart__layout">
        <div className="searchroomcalendarstart__monthYear">{`${date.getFullYear()}년 ${months[date.getMonth()]}월`}</div>
        <div className="searchroomcalendarstart__swiper">
          <button className="searchroomcalendarstart__swiper__left" onClick={prevMonth}>
            {'<'}
          </button>
          <button className="searchroomcalendarstart__swiper__right" onClick={nextMonth}>
            {'>'}
          </button>
        </div>

        {Week.map((day, index) => (
          <div className="searchroomcalendarstart__weekBox" key={index}>
            {day}
          </div>
        ))}
