...
const CalendarEnd = ({ onDayClick, setEndOpen }: CalendarEndType) => {
  ...
  return (
    <div className="calendarend">
      <div className="calendarend__layout">
        <div className="calendarend__monthYear">{`${date.getFullYear()}년 ${months[date.getMonth()]}월`}</div>
        <div className="calendarend__swiper">
          <button className="calendarend__swiper__left" onClick={prevMonth}>
            {'<'}
          </button>
          <button className="calendarend__swiper__right" onClick={nextMonth}>
            {'>'}
          </button>
        </div>

        {Week.map((day, index) => (
          <div className="calendarend__weekBox" key={index}>
            {day}
          </div>
        ))}
